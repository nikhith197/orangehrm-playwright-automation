...
test('@regression updates an employee and verifies the persisted value through API', async ({
  loginPage,
  dashboardPage,
  pimPage,
  employeeDetailsPage,
  ...
  await test.step('Update employee last name', async () => {
    await employeeDetailsPage.updateLastName(updatedEmployee.lastName);

    await expect(employeeDetailsPage.lastName).toHaveValue(updatedEmployee.lastName);

    await employeeDetailsPage.save();
  });

  await test.step('Verify updated employee through API', async () => {
    const apiEmployee = await apiClient.assertEmployeeExists(updatedName);

    expect(apiEmployee.lastName).toBe(updatedEmployee.lastName);
  });
  ...
  await test.step(`Search updated employee: ${updatedName}`, async () => {
    ...
    await employeeListPage.assertEmployeePresent(updatedEmployee);
  });
});
